...
* `connection_string` - параметр, строка подключения к базе данных. Указывается в ODBC формате, и обязательно должен содержать название драйвера для подключения к БД.
